...
public class BSONDocumentIterator: IteratorProtocol {
    ...
    internal func nextThrowing() throws -> BSONDocument.KeyValuePair? {
        ...
        let bson = try BSON.allBSONTypes[type]!.read(from: &buffer)
    ...
    internal func readNextType() throws -> BSONType? {
        ...
        guard let readBytes = self.buffer.readBytes(length: 1) else {
    ...
    /// Retrieves an ordered list of the keys in the provided document.
    internal static func getKeys(for document: BSONDocument) throws -> [String] {
        let iter = document.makeIterator()
        var keys = [String]()
        while true {
            guard let type = try iter.readNextType() else {
            ...
            let key = try iter.buffer.readCString()
            keys.append(key)
